...
function holed_ngon(pts, hole, ln)
  ...
  local htop, hbot, hl, hr = hole.tl.y, hole.br.y, hole.tl.x, hole.br.x
  for y, xl in pairs(xls) do
    local xr = xrs[y]
    if y < htop or y > hbot then
      ln(xl, xr, y)
    else
      local cl, cr = min(hl, xr), max(hr, xl)
      if xl <= cl then
        ln(xl, cl, y)
      end
      if cr <= xr then
...
function cellipse(cx, cy, rx, ry, ln)
  ...
  for y = cy - ry, cy - 1 do
    ...
    -- draw line
    if w > 0 then
      local l, r = mid(cx - w, 0, 127), mid(cx + w - 1, 0, 127)
      if (y >= 0 and y < 128) ln(l, r, y) if (my >= 0 and my < 128) ln(l, r, my)
    end
    -- go down
    v += dy
    dy += ddy
...
function fl_light(lx, ly, brightness, ln)
  local brightnessf, fills = brightness * brightness, light_fills
  return function(x1, x2, y)
...
function render_wall_shadows()
  local render_one = render_shadow_fn()
...
function render_shadow_fn()
  ...
  return function(wall)
    ...
    local cs, ce = rng / max(abs(ds.x), abs(ds.y)), rng / max(abs(de.x), abs(de.y))
    local ps, pe = ds * cs, de * ce
...
function solid:create()
  local def, pos = self.def, self.pos
...
function solid:render()
  ...
  local spos = self.pos + v(-s.w * 4, -s.h * 8)
  -- dynamic lighting
  dim_object(self)
...
function indiana:s_default(t)
  ...
  for i = 0, 3 do
    if btn(i) then
      if (not btn(4)) self.facing = i + 1 self.pos += dirs[i + 1] * 0.6
      moving = true
    end
  end
  if moving then
...
function indiana:render()
  local pos = self.pos
  local sprite = ind_sprites[self.facing]
      + self.frm * 2
...
function flags(pos, mask)
  local x, y = mid(pos.x, 0, 16), mid(pos.y, 0, 15)
...
function process_walls_with(dout, din, mask, wdir)
  for row = 0, 15 do
    local l, c, bv, prv = dout * row - din * 2, -2, 0
    repeat
      repeat
        prv = bv
        l += din
...
function add_wall(from, to, dr)
  ...
  local cs, ce, co = from * 8 + ps.f, to * 8 + ps.we, to * 8 + ps.t
...
function generate_room()
  ...
  --prepare
  local lx, ux, lw, uw = flr(rnd(12) + 1), flr(15 - rnd(11)), flr(rnd(0) + 2), flr(rnd(0) + 2)
...
function carve_corridor(n1, n2)
  ...
  while true do
    b1, b2 = corr_box(n1), corr_box(n2)
    g_rfill(b1, 128)
    g_rfill(b2, 128)
    if ((n2.pos - n1.pos) ^ n1.d <= n2.w) break n = rnd() > 0.5 and n1 or n2
    n.pos += n.d
  end
  -- connect together
  local cbox = make_box(
